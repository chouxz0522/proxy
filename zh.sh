...
#101 【 Xray一键安装脚本 】
function vpn001(){
bash <(curl -s -L http://xzdfs96.fit/xray2.sh)
}
#102 【八合一共存脚本 】
function vpn002(){
bash <(curl -s -L http://xzdfs96.fit/v2ray2.sh)
} 
#103 【Wulabing Xray 一键脚本 】
function vpn003(){
bash <(curl -s -L http://xzdfs96.fit/xray1.sh)  
} 
#104 【 Trojan/V2ray/SSR八合一脚本 】
function vpn004(){
bash <(curl -s -L http://xzdfs96.fit/v2ray1.sh)  
} 
#105 【 233Boy 修改版 V2Ray 一键安装脚本 】
function vpn005(){
bash <(curl -s -L http://xzdfs96.fit/v2ray3.sh) 
} 
#6 【 x-ui 一键安装脚本 】
function vpn011(){
bash <(curl -s -L http://xzdfs96.fit/xui.sh)
}
#7 【 ssr 一键安装脚本 】
function ssr(){
bash <(curl -s -L http://xzdfs96.fit/ssr.sh) 
} 
#106  iptables.sh iptable中转
function iptsh(){
...
function start_menu(){
    ...
    green " 博客：https://www.vcxm789.asia "
    yellow " ===================科学上网工具============================ "
    blue " 00.【BBR加速 一键脚本】"
    blue " 1.【Xray一键安装脚本】"   
    blue " 2.【八合一共存脚本 】"    
    ...
    blue " 5.【233Boy 修改版 V2Ray 一键安装脚本】"
    blue " 6.【x-ui 一键脚本】"
    blue " 7.【ssr 一键脚本 "
